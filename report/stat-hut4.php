...
require('conn.php');
	...
	$rs4 = mysql_query("SELECT COUNT(id) FROM tb_hut where comp='WAHYU JAYA PUSAT' ");
	$row4 = mysql_fetch_row($rs4);

	$rs8 = mysql_query("SELECT COUNT(id) FROM tb_hut where comp ='WAHYU JAYA PUSAT' and scan='2' ");
	...
	$val_book_hut	 = $row4[0]-$row8[0];

	$proc_booked_hut 	= number_format( (100* ($val_book_hut/$total_booked_hut) ),2)."%";
	$proc_used_hut		= number_format( (100* ($row4[0]/$total_booked_hut) ),2)."%";
		...
		<td width="50%">
			...
			<table width="90%" border="0">
			  ...
			  <!-<tr>
				...
				<td align="right"><?php echo $row4[0];?></td>
			  </tr>
			  <tr>
				<td width="60%"><strong>Jumlah Peserta Datang : </strong></td>
				<td align="right"><?php echo $row8[0];?></td>
			  </tr>
			  <tr>
				<td colspan="2"><hr/></td>
			  </tr>
